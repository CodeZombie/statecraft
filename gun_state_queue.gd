...
func _ready() -> void:
	...
	.transition_on("idle", "reload", self.is_magazine_pressed.bind())\
	...
	.add_state(
		...
		.add_enter_method(play_sound.bind(click_sound)) )\
	...
	.add_state(
		...
		.transition_on("idle", "clear_jam", self.is_magazine_pressed.bind())\
		.add_state(
	...
	.add_state(
		...
		.add_enter_method(play_sound.bind(reload_sound)))\
	...
	.add_exit_method(func(): self.loaded_rounds = self.magazine_capacity)\
	.on("mag_in", "mag_in.tween_finished", gun_state_machine.transition_to.bind("idle"))\
	.add_to_runner(gun_state_machine)
